...
export default function DangerAreaWarning({ onConfirm, onBypass, username }: DangerAreaWarningProps) {
  return (
    <div className="w-full max-w-md mx-auto bg-stone-900 border-4 border-red-500/80 rounded-3xl p-6 md:p-8 shadow-2xl relative overflow-hidden flex flex-col gap-5 text-center select-none font-mono">
      {/* 警告テクスチャ */}
      ...
      <div className="relative z-10 -mt-2 -mx-2 bg-amber-500/25 border border-amber-500/40 rounded-xl py-2 px-3 text-amber-300 font-sans text-[11px] leading-relaxed flex items-center justify-center gap-1.5 shadow-inner">
        <span>🎭</span>
        <span className="font-bold">お化け屋敷やホラーではありません。ギャグ・ネタ演出です！</span>
      </div>
      ...
      <div className="relative z-10 flex flex-col items-center gap-3 mt-1">
        <motion.div
          ...
          className="w-16 h-16 rounded-full bg-red-950 border-2 border-red-500 flex items-center justify-center text-red-500 relative"
        >
          <Skull className="w-8 h-8 text-red-500" />
          <span className="absolute -top-2.5 -right-5 bg-red-500 text-stone-950 text-[8px] font-sans font-bold px-1.5 py-0.5 rounded-md transform rotate-12 shadow-md">
            ネタだゾ
          </span>
        </motion.div>
        
        <h2 className="text-base md:text-lg font-bold text-red-500 tracking-wider">
          【 警告：精神汚染危険地区（※ネタ枠） 】
        </h2>
        
        <span className="text-[9px] bg-red-950/80 text-red-400 border border-red-500/30 px-3 py-0.5 rounded-full text-center">
          SYSTEM: COMEDY ZONE TRANSITION
        </span>
      </div>

      <div className="relative z-10 space-y-3 bg-stone-950/80 border border-red-950/60 p-4 rounded-2xl text-left font-sans text-stone-300">
        <p className="text-xs leading-relaxed">
          被験者 <strong className="text-white font-mono">{username || 'みつき'}</strong>、通常フェーズの幾何学観測が終了しました。
        </p>
        <p className="text-xs leading-relaxed">
          ここから先は、案内人ダーリンちゃん、LSI知識芋虫、ご褒美くん（IEI）、およびLII作者が実在する<span className="text-red-400 font-bold">非同期LINEチャット次元（ネタ爆笑攻撃地区）</span>となります。
        </p>
        ...
        <p className="text-[11px] font-bold text-red-400/90 text-center border-t border-red-950/60 pt-2.5">
