...
export default function RobotBuilder({ onComplete, username }: RobotBuilderProps) {
  ...
  // コンフィグ説明
  const headDocs = {
    visor: { name: '🔴 水平式オプティカル・バイザー', desc: '広角パノラマ知覚。感覚（Se/Si）の検出プロトコルの優先。' },
    camera: { name: '👁️ 単眼同軸フォーカス・カメラ', desc: '局所的、精緻なターゲット走査。内向論理（Ti）に直結。' },
    monocle: { name: '📐 幾何学アナライズ・モノクル', desc: '構造的・多次元情報から未来予測を演算（Ni/Ne選好）。' },
  };

  const coreDocs = {
    quantum: { name: '💎 量子多世界コプロセッサ', desc: '確率の重ね合わせ状態を直接並列処理（C:クリエイター優位）。' },
    steam: { name: '🔥 熱力学高圧エンタルピー釜', desc: '推進意志、物理圧力エネルギーの直接出力（D:ドミナント優位）。' },
    fluidic: { name: '💧 流体パリティ調和コンデンサ', desc: '環境低周波と水位を静寂同調させる（H:ハーモナイザー選好）。' },
  };

  const armDocs = {
    cannon: { name: '💥 圧縮粒子パルス・キャノン', desc: '物理的作用・強制同期用（Se/Te出力型アーム）。' },
