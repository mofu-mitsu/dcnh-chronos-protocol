  ...
  const getSelfTypeAnalysis = (self: string, main: string, sub: string) => {
    ...
    if (isNone) {
      const generalAnalyses: Record<string, string> = {
        'D': '【独立意思の開拓者（ドミナント特化型）】\n特定のソシオタイプに囚われず、非常に強固な実践エネルギー（Te / Se）を主軸に置く、圧倒的なアクティブ・リーダータイプです！ビジョンに向けた突破力があり、周囲が「バグかも？」と尻込みする困難や例外事項にも、自ら物理的な手を突っ込んで解決する強力な主導力を秘めています。',
        'C': '【カオスの探求者（クリエイター特化型）】\n自認の枠に収まらない、無限の自由さと発想の柔軟性（Ne / Se）を誇るオルタナティブ・ハッカータイプです！退屈なルールや固定観念の枠（カレンダー）をハサミで切り刻み、全く新しい体験や彩りを生み出します。遊び心と、バグすら楽しみながら進化させる柔軟なハック精神があります。',
        'N': '【一貫性の守護者（ノーマライザー特化型）】\n特定のタイプ枠を超え、論理の再現性と客観的な正しさ（Ti / Fi）を追求し続ける最高精度デバッガータイプです！周囲の感情レベルのノイズに流されず、「何が再現可能で、何が定義上一貫しているか」を冷徹かつ美しいクラス図に分類し、完璧なアプローチで調和させます。',
        'H': '【静寂の観測調律師（ハーモナイザー特化型）】\n特定の構造的役割を持たずとも、その場にいる人々のピリピリ感を消音（Fi / Ni）し、物事を優しく受け流すことができるマスター・バランサータイプです！時間経過とシンクロしながら余白と安らぎをもたらし、あらゆる例外処理エラーも穏やかな知性で静かに調和（デバッグ）させます。',
      };
    ...
    const analyses: Record<string, string> = {
      // 🧊 アルファ・クアドラ
      'LII-N': '【完全精神静音同期】LII（論理・組織）✕ ノーマライザー（規範型）。LII研究者の最も自然で強靭な究極進化系です！あらゆる不条理さ・不正確さという「バグ」に対して、冷静沈着にデバッグ措置を施す客観世界のガーディアン。芋虫（LSI）が「ふむ、さすがの精密さだな」と小さくヤキモチを妬きながら頷くほど、論理構造の美しさと時間の正確さを極限状態で維持できます。',
      ...
      // ⚔️ ベータ・クアドラ
      'IEI-C': '【叙情カオスハッカー】IEI（叙情家）✕ クリエイター（創造型）。ご褒美くん（IEI）も大興奮の直感お遊び領域です！言葉の泡、気まぐれなエモい衝動、そして予測不可能な逆襲ハックを操って、退屈な日常のフレームワークをバグらせます。「お風呂上がりの出汁豚骨をわしゃわしゃする」ような、愛され気質なぶっ飛び加減で天才的に世界を彩るタイプ。',
