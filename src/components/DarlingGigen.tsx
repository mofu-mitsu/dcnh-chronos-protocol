...
export default function DarlingGigen({ question, onAnswer, onLogAction }: DarlingGigenProps) {
  ...
  const handleLoveSubmit = () => {
    ...
    // 「別に」(D+2)
    if (/別に|べつに|ベツニ|ベツに/i.test(text)) {
      score.D += 2;
      response = '「『別に』って、うわあああっツレなーい！でもその突き放すようなツンツンした態度、まさにドミナント（D：主導）の強者感があって、ウチをMっぽくバグらせる気やね！？大好物やん…♡」';
      onLogAction?.(`Feインターフェース「私のこと好き？」への回答: [${text}] -> 別に/主導拒絶反応 (D+2)`);
    }
    // 「ふつう」「まあまあ」(H+1, N+1)
    else if (/ふつう|普通|まあまあ|マーマー|フツウ|マアマア/i.test(text)) {
      score.H += 1;
      score.N += 1;
      response = '「『ふつう』とか『まあまあ』とか…！ちょっと、そこはもっとテンション上げて『大好き』って言ってよー！でも、その波風立てないフラットで穏やかなローテンションさ、まさに調和(H)と規範(N)のブレンドって感じで、じわじわ愛せるんよね…♡（ウシシ）」';
      onLogAction?.(`Feインターフェース「私のこと好き？」への回答: [${text}] -> 中庸調和反応 (H+1, N+1)`);
    }
    // キモ系 (Seワード - 拒絶・ツッコミ) -> ドミナント優位（D+2） + クリエイター（C+1）
    else if (/キモ|きも|いいえ|やだ|嫌|ウザ|NO|no|うんこ|反対|好きじゃない|でも思ったか|うざ|きしょ|キショ|死ね|しね|無理|むり|嫌い|きらい|うっとう|うるさ|黙|拒絶|きもい|キモい/i.test(text)) {
      score.D += 2;
      score.C += 1;
    ...
    // 構築系・お遊び系 (クリエイター)
      ...
      score.C += 2;
      score.H += 1;
      response = `「『${text}』だって！その想定外 of 想定外なワードチョイス、さすがクリエイター（C：創造）やん！ウチのロジックがバグるくらい面白いこと言ってくるダーリン、やっぱり飽きひんわぁ♡」`;
