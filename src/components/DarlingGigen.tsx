...
export default function DarlingGigen({ question, onAnswer, onLogAction }: DarlingGigenProps) {
  ...
  const [tsuigekiCount, setTsuigekiCount] = useState(0); // 追撃のプレッシャー(Se値)向上バッファ
  ...
  useEffect(() => {
    ...
    intervalRef.current = window.setInterval(() => {
      ...
      if (secondCounter >= 10) {
        ...
        setTsuigekiCount((prev) => prev + 1); // 追撃した分Seスコア（D）をブースト！
        onLogAction?.('ダーリンちゃんの放置追撃LINEが発火した (プレッシャーSe/D+1)');
      }
    }, 1000);
  ...
  const handleLoveSubmit = () => {
    ...
    // キモ系 (Seワード - 拒絶・ツッコミ) -> ドミナント優位（D+4） + クリエイター（C+2）
    if (/キモ|きも|いいえ|やだ|嫌|ウザ|うんこ|反対|好きじゃない|でも思ったか|うざ|きしょ|キショ|死ね|しね|無理|むり|嫌い|きらい|うっとう|うるさ|黙|拒絶|きもい|キモい/i.test(text)) {
      score.D += 4;
      score.C += 2;
      response = '「きゃっ♡ 『きもい』とか『無理』とか、そういう辛口なツッコミ大好物なんよ！さすがダーリン、Se（感覚）の圧が強くてゾクゾクするやん！ウシシ♡」';
      onLogAction?.(`Feインターフェース「私のこと好き？」への回答: [${text}] -> Se/ツッコミ反応 (D+4, C+2)`);
    }
    // 誰？系 (冷淡・現実主義・境界設定) -> ノーマナイザー
      ...
      onLogAction?.(`Feインターフェース「私のこと好き？」への回答: [${text}] -> 冷徹境界反応 (N+4)`);
    }
    // 甘い系 (好き・愛・共感) -> ハーモナイザー
    else if (/好き|愛|すき|あい|ちゅ|ちゅっ|ハグ|うん|はい/i.test(text)) {
      score.H += 4;
      response = '「えへへ♡ 『好き』って言ってくれたああア！嬉しい！ウチ、ダーリンのそういう優しいハーモナイザー（H：調和）なところ、本当に大好物なんやから♡ ギュってして！」';
      onLogAction?.(`Feインターフェース「私のこと好き？」への回答: [${text}] -> 甘甘共感反応 (H+4)`);
    }
    // 構築系・お遊び系 (クリエイター)
    else {
      score.C += 4;
      ...
      onLogAction?.(`Feインターフェース「私のこと好き？」への回答: [${text}] -> 想定外創造反応 (C+4, H+1)`);
    }
  ...
  const handleNext = () => {
    if (selectedOpt) {
      ...
      // 放置追撃回数分だけ、被験者みつきのSe数値(D)を優位にブーストし、クリエイター（C：Se接続遊び心）も連動加算！
      if (tsuigekiCount > 0) {
        finalScore.D = (finalScore.D || 0) + (tsuigekiCount * 3);
        finalScore.C = (finalScore.C || 0) + (tsuigekiCount * 2);
      }
      ...
      onLogAction?.(`ダーリン最終解答完了: 追撃D+${tsuigekiCount * 3}/C+${tsuigekiCount * 2}, Feボーナス(${JSON.stringify(loveScoreBonus)})を統合完了`);
      onAnswer(finalScore);
    }
  };
